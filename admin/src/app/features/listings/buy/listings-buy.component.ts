...
import { BuyerProductListing, ProductStatusType, ProductModerationLogItem } from '../../../core/models/listings.model';
  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-violet-50/20 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">Publicaciones de Compra</h1>
          <p class="mt-1 text-sm font-medium text-slate-500">
            Administra, audita y rastrea las solicitudes de compra del marketplace.
          </p>
        </div>
      ...
      <div class="mb-6 flex flex-col gap-3 sm:flex-row">
        <div class="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar por artículo buscado..."
            ...
            aria-label="Buscar solicitud de compra"
          />
        </div>
        <button
          type="button"
      ...
      @if (loading()) {
        <div class="flex min-h-[300px] items-center justify-center rounded-2xl border border-slate-100 bg-white/70 shadow-sm backdrop-blur-md">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-slate-500">Cargando solicitudes...</span>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-100 bg-white/70 py-16 text-center shadow-sm backdrop-blur-md">
          <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-indigo-50 text-indigo-400">
            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 002-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
            </svg>
          </div>
          <h3 class="text-sm font-bold text-slate-800">No hay solicitudes</h3>
          <p class="mt-1 text-sm text-slate-500 max-w-xs mx-auto">No se encontraron solicitudes que coincidan con los criterios actuales.</p>
        </div>
      } @else {
        <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white/70 shadow-sm backdrop-blur-md">
          <div class="overflow-x-auto">
            <table class="w-full text-sm" aria-label="Lista de solicitudes de compra">
              <thead>
                <tr class="border-b border-slate-100 bg-slate-50/80">
                  <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Solicitud</th>
                  <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Comprador</th>
                  <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Presupuesto</th>
                  <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Cantidad</th>
                  <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider text-slate-500">Ofertas</th>
              ...
              <tbody>
                @for (item of listings(); track item.id) {
                  <tr class="border-b border-slate-50 transition-colors hover:bg-indigo-50/40">
                    ...
                    <td class="px-4 py-3">
                      <div class="font-semibold text-slate-800">{{ item.user?.name || 'Comprador' }} {{ item.user?.last_name || '' }}</div>
                      <div class="text-xs text-slate-500 mt-0.5">{{ item.email || item.user?.email || 'N/A' }}</div>
                    </td>
                    <td class="px-4 py-3">
                      <div class="font-semibold text-slate-800">
                        {{ item.min_price | currency : item.currency || 'MXN' : 'symbol' : '1.0-0' }}
                        –
                        {{ item.max_price | currency : item.currency || 'MXN' : 'symbol' : '1.0-0' }}
                      </div>
                      @if (item.limit_max_price) {
                        <span class="mt-0.5 inline-flex rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-600 ring-1 ring-rose-200">
                          Límite estricto
                        </span>
                      }
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                      <span class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-bold text-indigo-700 ring-1 ring-indigo-200">
                        {{ item.offers_count || 0 }}
                      </span>
                    </td>
                    <td class="px-4 py-3 text-center">
          ...
          <div class="flex-1 space-y-4 overflow-y-auto px-5 py-5">
              ...
              <div class="relative flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="truncate text-sm text-white/75">
                    {{ detail.min_price | currency : detail.currency || 'MXN' : 'symbol' : '1.0-0' }}
                    –
                    {{ detail.max_price | currency : detail.currency || 'MXN' : 'symbol' : '1.0-0' }}
                  </p>
                </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white/70 shadow-sm backdrop-blur-md">
              ...
              <div class="grid grid-cols-2 gap-4 p-4 text-sm">
                ...
                <div>
                  <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Marca preferida</span>
                  <span class="font-semibold text-slate-800">{{ detail.brand || 'Cualquiera' }}</span>
                </div>
                <div>
                  <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Modelo</span>
                  <span class="font-semibold text-slate-800">{{ detail.model || 'Cualquiera' }}</span>
                </div>
                <div>
                ...
                <div>
                  <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Cantidad</span>
                  <span class="font-semibold text-slate-800">{{ detail.quantity }} un.</span>
                </div>
                <div>
                  <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Ofertas</span>
                  <span class="font-semibold text-slate-800">{{ detail.offers_count || 0 }}</span>
                </div>
                <div class="col-span-2">
                  <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Comprador</span>
...
export class ListingsBuyComponent implements OnInit {
  ...
  selectedItem = signal<BuyerProductListing | null>(null);
  ...
  private openListingFromQuery(id: number): void {
    ...
    this.listingsService.getBuyerListingById(id).subscribe({
      next: (res) => {
  ...
  openDetail(item: BuyerProductListing): void {
  ...
  loadHistory(productId: number): void {
    ...
    this.listingsService.getListingModerationHistory('buyer', productId).subscribe({
      next: (res) => {
  ...
  changeStatus(item: BuyerProductListing, newStatus: ProductStatusType): void {
    let actionType: 'delete' | 'moderate' | 'activate' = 'moderate';
    let title = 'Confirmar Moderación';
    let message = `¿Deseas cambiar el estado de la solicitud #${item.id} a "MODERADO"?`;

    if (newStatus === 'deleted') {
      actionType = 'delete';
      title = 'Confirmar Eliminación';
      message = `¿Deseas marcar la solicitud #${item.id} como "ELIMINADO"?`;
    } else if (newStatus === 'active') {
      actionType = 'activate';
      title = 'Aprobar Solicitud';
      message = `¿Deseas aprobar y activar la solicitud #${item.id} para que vuelva a ser visible públicamente?`;
    }
  ...
  executeConfirmedAction(): void {
    ...
    this.listingsService
      .updateListingStatus('buyer', itemId, targetStatus, reason, notes)
      .subscribe({
        next: () => {
          this.loadData();
